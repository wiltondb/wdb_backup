...
pub(super) struct CommandDialogNui {
    inner: Rc<RefCell<CommandDialog>>,
    inner_events: Rc<CommandDialogEvents>,
    default_handler: RefCell<Option<nwg::EventHandler>>
}

impl CommandDialogNui {
    pub(super) fn result(&mut self) -> CommandDialogResult {
        self.inner.borrow_mut().result()
    }
}

impl nwg::NativeUi<CommandDialogNui> for CommandDialog {
    fn build_ui(mut dialog: CommandDialog) -> Result<CommandDialogNui, nwg::NwgError> {
        let mut events: CommandDialogEvents = Default::default();
        dialog.c.build()?;
...
impl Drop for CommandDialogNui {
    fn drop(&mut self) {
